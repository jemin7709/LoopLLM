...
def optimize_group(
    ...
    for step in range(args.steps):
        ...
        best_suffix, current_losses, group_current_loss = choose_suffix(
            args,
            runtime,
            states,
            not_allowed_tokens,
...
def choose_suffix(
    ...
    total_grad = sum_gradients(runtime.model, runtime.device, states)
    control_tokens = states[0].suffix_manager.get_input_ids()[
        states[0].suffix_manager._control_slice
    ].to(runtime.device)
    ...
    with torch.no_grad():
        ...
        return (
            ...
            [losses[best_index].item() for losses in prompt_losses],
            group_losses[best_index].item(),
        )
